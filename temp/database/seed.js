...
async function seedDatabase() {
    ...
    `);
        ...
        // Create principal
        const principal = await (0, auth_1.createUser)({
            username: 'principal',
        ...
        const teacher1 = await (0, auth_1.createUser)({
            username: 'teacher1',
        ...
        const teacher2 = await (0, auth_1.createUser)({
            username: 'teacher2',
        ...
        const student1 = await (0, auth_1.createUser)({
            username: 'student1',
        ...
        const student2 = await (0, auth_1.createUser)({
            username: 'student2',
        ...
        const subjectStmt = database_1.db.prepare(`
    ...
    `);
        ...
        const classStmt = database_1.db.prepare(`
    ...
    `);
        ...
        const teacherStmt = database_1.db.prepare(`
    ...
    `);
        ...
        const studentStmt = database_1.db.prepare(`
    ...
    `);
        ...
        const firstClass = database_1.db.prepare('SELECT id FROM classes WHERE name = ?').get('Grade 9A');
        ...
        const teacherSubjectStmt = database_1.db.prepare(`
      INSERT INTO teacher_subjects (teacher_id, subject_id, class_id) VALUES (?, ?, ?)
    `);
        // Get teacher record IDs
        const teacher1Record = database_1.db.prepare('SELECT id FROM teachers WHERE user_id = ?').get(teacher1.id);
        const teacher2Record = database_1.db.prepare('SELECT id FROM teachers WHERE user_id = ?').get(teacher2.id); // Teacher 1 teaches Math and English to Class 1
        ...
        const timetableStmt = database_1.db.prepare(`
    ...
    `);
        ...
        const announcementStmt = database_1.db.prepare(`
    ...
    `);
        ...
        for (const announcement of announcements) {
            announcementStmt.run(announcement.title, announcement.content, announcement.author_id, announcement.target_audience, announcement.priority, announcement.expires_at);
        } // Create sample leave requests
        console.log('Creating sample leave requests...');
        const leaveRequestStmt = database_1.db.prepare(`
    ...
    `);
        ...
        for (const request of leaveRequests) {
            leaveRequestStmt.run(request.user_id, request.leave_type, request.start_date, request.end_date, request.reason, request.status, request.created_at, request.approved_by, request.approval_date, request.approval_comments);
        }
